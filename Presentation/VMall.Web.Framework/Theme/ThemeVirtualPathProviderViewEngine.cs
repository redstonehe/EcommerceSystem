...
namespace VMall.Web.Framework
{
    ...
    public abstract class ThemeVirtualPathProviderViewEngine : VirtualPathProviderViewEngine
    {
        ...
        /// PC店铺前台视图位置格式
        /// </summary>
        private readonly string[] _storewebviewlocationformats = new string[1] { "~/themes/{2}/views/{0}.cshtml" };
        ...
        /// 代理前台视图位置格式
        /// </summary>
        private readonly string[] _agentviewlocationformats = new string[2] { "~/Agentsystem/views/{1}/{0}.cshtml", "~/Agentsystem/views/shared/{0}.cshtml" };

        /// <summary>
        /// 汇购优选前台视图位置格式
        /// </summary>
        private readonly string[] _yxviewlocationformats = new string[2] { "~/youxuan/views/{1}/{0}.cshtml", "~/youxuan/views/shared/{0}.cshtml" };
        ...
        private string GetPath(ControllerContext controllerContext, string name, string controllerName, string cacheKeyPrefix, bool useCache, out string[] searchedLocations)
        {
            ...
            else if (area == "malladmin")//PC商城后台视图位置的处理
            {
                ...
                List<string> strList = new List<string>(_malladminviewlocationformats);
                strList.Add("~/plugins/VMall.PayPlugin.WeChat/views/{1}/{0}.cshtml");//plugins\VMall.PayPlugin.WeChat\views\adminwechat
                locations = strList.ToArray();
            }
            else if (area == "agent")//移动商城代理系统视图位置的处理
            {
                locations = _agentviewlocationformats;
            }
            else if (area == "yx")
            {
                locations = _yxviewlocationformats;
            }
            //是否为特殊路径的标识
