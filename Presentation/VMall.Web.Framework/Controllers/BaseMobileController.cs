...
namespace VMall.Web.Framework
{
    ...
    public class BaseMobileController : Controller
    {
        ...
        protected override void OnActionExecuting(ActionExecutingContext filterContext)
        {
            ...
            //filterContext.Result = new RedirectToRouteResult("moblie_error", new RouteValueDictionary { { "ssuid", "11" } },false);


            List<string> allowConntrols = new List<string>() ;
            allowConntrols.Add("test");
            allowConntrols.Add("store");
            allowConntrols.Add("orderview");
            allowConntrols.Add("error");
            allowConntrols.Add("codeimg");
            allowConntrols.Add("category");
            allowConntrols.Add("catalog");
            allowConntrols.Add("help");

            if (allowConntrols.Exists(x => x == RouteData.Values["controller"].ToString().ToLower()))
            {
                //未登陆用户获取querystring传过来的from_uid 并保存入cookie
                if (WorkContext.Uid < 1)
                {
                    var from_uid = filterContext.RequestContext.HttpContext.Request.QueryString.Get("from_uid");
                    if (from_uid != null)//存在来源推荐Uiduid，写入缓存
                    {
                        int cookpuid = TypeHelper.StringToInt(WebHelper.GetCookie("puid"));
                        if (cookpuid <= 0)
                            //不登录情况下保存url的uid来锁定推荐关系
                            WebHelper.SetCookie("puid", from_uid, 60 * 24 * 30);
                    }
                }
                // 已登陆用户querystring增加from_uid 写入分享来源
                if (WorkContext.Uid > 0)
                {
                    if (filterContext.RequestContext.HttpContext.Request.QueryString.Get("from_uid") == "" || filterContext.RequestContext.HttpContext.Request.QueryString.Get("from_uid") == null)
                    {
                        RouteValueDictionary vauleList = new RouteValueDictionary {
                        { "action", RouteData.Values["action"].ToString() },
                        { "controller", RouteData.Values["controller"].ToString() },
                        { "area", RouteData.Values["area"].ToString() },
                        { "from_uid", WorkContext.Uid}
                     };
                        foreach (var item in filterContext.RequestContext.HttpContext.Request.QueryString.AllKeys)
                        {
                            vauleList.Add(item, filterContext.RequestContext.HttpContext.Request.QueryString[item]);
                        }
                        filterContext.Result = new RedirectToRouteResult(vauleList);
                    }
                }
            }
            

            //filterContext.Result = RedirectToRoute(new { controller = RouteData.Values["controller"].ToString(), action = RouteData.Values["action"].ToString(), area = RouteData.Values["area"].ToString(), godd = "666" });
        ...
        protected override void OnException(ExceptionContext filterContext)
        {
            ...
            LogHelper.WriteOperateLog("商城手机端", "错误信息: 来源地址：" + urlReferrer + "  ,当前地址：" + filterContext.HttpContext.Request.Url.AbsoluteUri, "异常方法" + filterContext.Exception.TargetSite + ",\r\n异常信息:" + filterContext.Exception.ToString() + " ,访问者IP：" + WebHelper.GetIP(), (int)LogLevelEnum.ERROR);
            if (WorkContext.IsHttpAjax)
            ...
            else
            {
                ...
                filterContext.HttpContext.Response.Redirect("/mob/error.html");

                //filterContext.HttpContext.Response.Redirect("/mob");
                //filterContext.Result = new ViewResult() { ViewName = "error" };
                //filterContext.ExceptionHandled = true;
                ////filterContext.HttpContext.Response.Status = "Not Found";
                //filterContext.HttpContext.Response.StatusDescription = "Not Found";
                ...
                //filterContext.HttpContext.Response.Redirect("/mobile/error.html");
                //filterContext.Result = View("~/mobile/views/shared/error.cshtml");
