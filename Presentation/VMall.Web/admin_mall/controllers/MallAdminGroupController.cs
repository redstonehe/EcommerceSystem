...
namespace VMall.Web.MallAdmin.Controllers
{
    ...
    public partial class MallAdminGroupController : BaseMallAdminController
    {
        ...
        public ActionResult List()
        {
            ...
            {
                MallAdminGroupList = MallAdminGroups.GetMallAdminGroupList()
            };
        ...
        public ActionResult Edit(int mallAGid = -1)
        {
            ...
            MallAdminGroupModel model = new MallAdminGroupModel();
            model.MallAGid = mallAdminGroupInfo.MallAGid;
        ...
        /// <returns></returns>
        public ActionResult GroupUserlist(int mallAGid = -1) {
            if (mallAGid < 2)
                return PromptView("非商城管理员组");

            MallAdminGroupInfo mallAdminGroupInfo = MallAdminGroups.GetMallAdminGroupById(mallAGid);
            if (mallAdminGroupInfo == null)
                return PromptView("商城管理员组不存在");
            List<PartUserInfo> list = AdminUsers.GetAdminGroupUserList(mallAGid);
            return View(list);
        }
    }
}
