...
$(function () {
    ...
    /*更改编辑按钮样式*/
    //$("#example2 tbody tr td a[class='editOperate'],#example2 tbody tr td a[class='deleteOperate']").addClass("btn btn-primary btn-sm");
   
    $("#example2 tbody tr td:last-child a,#example2 tbody tr td:last-child span").addClass("btn btn-primary btn-sm");

    //$(".rightH1 .right a img").remove();
    //$(".rightH1 .right a").removeClass("menuBT").addClass("btn btn-primary btn-sm border-radius-lg").prepend("<i class='fa fa-plus-circle fa-lg'></i>");
    ...
    //批量备货
    $(".batchPreProduct").click(function () {
        if ($("input[type='checkbox'][selectItem='true']:checked").length > 0) {
            if (confirm("您确定要备货全部选择订单吗？")) {
                doPostBack($(this).attr("delUrl"));
            }
        }
        else {
    ...
    $(".batchShow").click(function () {
        if ($("input[type='checkbox'][selectItem='true']:checked").length > 0) {
            if (confirm("您确定要显示全部吗？")) {
                doPostBack($(this).attr("showUrl"));
            }
        }
        else {
            alert("没有选中任何一项!");
    ...
    //批量提现完成
    $(".batchDrawSuccess").click(function () {
        if ($("input[type='checkbox'][selectItem='true']:checked").length > 0) {
            if (confirm("您确定要批量完成所有提现申请？")) {
                doPostBack($(this).attr("delUrl"));
            }
        }
        else {
