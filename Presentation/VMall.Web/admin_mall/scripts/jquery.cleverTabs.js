...
function CleverTabs(self, options) {
    ...
    this.options = $.extend({
        ...
        //Tab用于控制的头模板
        tabHeaderTemplate: '<li id="cleverTabHeaderItem-#{id}" style="padding: 0 20px 0 0;" class="#{liclass}"><a href="#" title="#{title}">#{label}</a></li>',
        //Tab用于显示的Panel模板
    ...
    if (this instanceof CleverTabs) {
        CleverTabs.prototype.resizePanelContainer = function () {
            if (pc.attr('id').indexOf('cleverTabsPanelContainer') === 0) {
                var height = self.height() - el.height()-10;
...
CleverTabs.prototype.add = function (options) {
    ...
    if (self.panelContainer.attr('id').indexOf('cleverTabsPanelContainer') === 0) {
        var height = self.wrapper.height() - self.element.height()-10;
...
CleverTabs.prototype.setupContextMenu = function () {
    ...
    if (!self.options.contextMenu) {
        ...
        + '<li id="mnuEnabl" ><a href="#enabled"><span class="ui-icon ui-icon-pencil" style="float: left; margin-right: 5px;"></span>开启</a></li>'
...
CleverTab.prototype.kill = function () {
    ...
    if (self.tabs.panelContainer.attr('id').indexOf('cleverTabsPanelContainer') === 0) {
        var height = self.tabs.wrapper.height() - self.tabs.element.height()-10;
...
CleverTab.prototype.setLock = function (lock, changeOrgLock) {
    ...
    if (!lock) {
        ...
        //刷新按钮
        var btnRefresh = $('<a href="javascript:void(0)" class="ui-corner-all" title="Refresh" style="position: absolute; cursor: pointer; padding: 0px; top: 1px; right: 16px"><span class="ui-icon ui-icon-refresh"></span></a>');
        this.header.append(btnRefresh);
        btnRefresh.bind('mouseover', function () {
            $(this).addClass('ui-state-active');
        });
        btnRefresh.bind('mouseout', function () {
            $(this).removeClass('ui-state-active');
        });
        btnRefresh.bind('click', function () {
            new CleverTab(self.tabs, self.id).refresh();
        });
    } else {
