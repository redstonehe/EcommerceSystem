...
function selectCommonAttrValue(obj) {
    ...
    if (productPageType == 1) {
        ...
        if (attrValueObj.attr("checked") == false) {
            changeProductAttributeUrl = "/malladmin/product/delproductattribute?pid=" + pid + "&attrId=" + attrValueObj.parent().find("input[name=AttrIdList]").val() + "&t=" + new Date();
        }
        else {
            if (isInput == "0") {
                changeProductAttributeUrl = "/malladmin/product/updateproductattribute?type=0" + "&pid=" + pid + "&attrId=" + attrValueObj.parent().find("input[name=AttrIdList]").val() + "&attrValueId=" + attrValueObj.val() + "&t=" + new Date();
            }
        }
...
//构建SKU属性选择表格
function buildSKUAttrSelectTableForSingle(result) {
    if (result.length < 1) {
        return "";
    }
    ...
    for (var i = 0; i < result.length; i++) {
        ...
        for (var j = 0; j < result[i].attrValueList.length; j++) {
            if (result[i].attrValueList[j].isInput == "0") {
                tableHtml += "<label><input type='radio' name='attrValue' attrId='" + result[i].attrId + "' attrValue='" + result[i].attrValueList[j].attrValue + "' onclick='selectSKUAttrValue()' value='" + result[i].attrValueList[j].attrValueId + "' />" + result[i].attrValueList[j].attrValue + "</label>" + "&nbsp;&nbsp;";
            }
            else {
                tempInputAttrValueId = result[i].attrValueList[j].attrValueId;
            }
...
function setSelectedStore(item) {
    var storeId = parseInt($(item).attr("storeId"));
    $(openStoreSelectLayerBut).parent().find(".StoreId").val(storeId);
    ...
    if (productPageType != 2) {
        ...
        $.get("/malladmin/store/storeclassselectlist?storeId=" + storeId + "&time=" + new Date(), function (data) {
            var result = eval("(" + data + ")");
            var html = "";
            html += "<option value='-1' slected='selected'>选择店铺分类</option>";
            for (var i = 0; i < result.length; i++) {
                html += "<option value='" + result[i].id + "'>" + result[i].name + "</option>";
            }
            $("#StoreCid").html(html).attr("disabled", "");
        });

        $("#StoreSTid").html("").attr("disabled", "disabled");
        $.get("/malladmin/store/storeshiptemplateselectlist?storeId=" + storeId + "&time=" + new Date(), function (data) {
            var result = eval("(" + data + ")");
            var html = "";
            html += "<option value='-1' slected='selected'>选择配送模板</option>";
            for (var i = 0; i < result.length; i++) {
                html += "<option value='" + result[i].id + "'>" + result[i].title + "</option>";
            }
            $("#StoreSTid").html(html).attr("disabled", "");
        });

        if ((productPageType == 0 || productPageType == 3) && storeId > 0) {
            $("#descriptionLi").show();
            $("#mobiledescriptionLi").show();
        }
    }
}
...
$(function () {
    ...
    //初始化店铺分类选择框
    if (productPageType != 2 && storeId > 0) {
        ...
        $.get("/malladmin/store/storeshiptemplateselectlist?storeId=" + storeId + "&time=" + new Date(), function (data) {
            ...
            $("#StoreSTid").html(html).attr("disabled", "");
            $("#StoreSTid").find("option[value=" + $("#hiddenStoreSTid").val() + "]").attr("selected", true);
        });
    }
    ...
    if (productPageType != 2 && cateId > 0) {
        ...
        if (productPageType == 0 || productPageType == 1) {
            ...
            $.get("/malladmin/category/aandvjsonlist?cateId=" + cateId + "&time=" + new Date(), function (data) {
                ...
                //});
                $(document).on("focus", ".inputattrvalue", function () {
                    oldInputAttrValue = $(this).val();
                });
                $(document).on("blur", ".inputattrvalue", function () {
                //$(".inputattrvalue").live("blur", function () {
                    var inputAttrValueObj = $(this);
                    if (inputAttrValueObj.val() != oldInputAttrValue) {
                        $.jBox.tip("正在更新...", 'loading');
                        $.get("/malladmin/product/updateproductattribute?type=1" + "&pid=" + pid + "&attrId=" + inputAttrValueObj.parent().find("input[name=AttrIdList]").val() + "&attrValueId=" + inputAttrValueObj.attr("attrValueId") + "&inputValue=" + encodeURIComponent(inputAttrValueObj.val()) + "&t=" + new Date(), function (data, textStatus) {
                            if (data != "0") {
                                $.jBox.tip('更新成功！', 'success');
                            } else {
