...
function OrgNode() {
    ...
    this.inIt = function () {
        ...
        document.getElementById(this.TargetDivId).appendChild(tempDiv);
...
function OrgShow(OrgNode_) {
    ...
    //this.TotalWidth = ChildNodesWidth(OrgNode_);

    this.Run = function () {
        //var showDiv = document.createElement("DIV");
        //showDiv.id = "showOrg";
        //showDiv.style.cssText = "border:2px red solid;color:#f00;height:2px;";
        ...
        //this.TotalWidth = ChildNodesWidth(OrgNode_);
        //this.Left = (document.documentElement.clientWidth / 2+60) - this.TotalWidth / 2;
        //***************************
        ...
        for (var n = 0; n < this.Nodes.length; n++) {//重新设置距离
            ...
            this.Nodes[n].Box.style.left = this.Nodes[n].Left + 2 + "px"
            ...
            for (var m = 0; m < tempNodes.length; m++) {
                if (n != this.Depth) {//设置底线条
                    if (tempNodes[m].Nodes.length != 0) {
                        ...
                        var tempBottomLine = new CreateLine(2, tempLineTop, tempLineLeft, tempLineHeight, this.LineSize, this.LineColor, "LineBottom_" + tempNodes[m].Id, this.TargetDivId);
                ...
                if (n != 1) {//设置顶线条
                    ...
                    if (this.DepthGroup[tempNodes[m].Depth].NodeGroups[tempNodes[m].NodeGroupId].length == 1) {//如果只有一个节点
                        ...
                        var temptopLine = new CreateLine(2, tempLineTop, tempLineLeft, tempLineHeight, this.LineSize, this.LineColor, "LineTop_" + tempNodes[m].Id, this.TargetDivId);
        ...
        for (var n = 2; n <= this.Depth; n++) {//设置横线条
            ...
            for (var m = 0; m < tempNodeGroups_.length; m++) {
                if (tempNodeGroups_[m].length != 1) {
                    ...
                    var tempGroupLine = new CreateLine(1, tempLineTop, tempLineLeft, tempLineWidth, this.LineSize, this.LineColor, "LineGroup_" + tempNodeGroups_[m][0].Id,this.TargetDivId);
                }
            }
        }
    ...
    function BoxInit(OrgObj_) {//节点初始化
        ...
        OrgObj_.TargetDivId = This.TargetDivId;
        OrgObj_.inIt();
    ...
    function GetBoxTemplet() {//取得节点模板
        ...
        var TempletStr = "<div id=\"{Id}\" style=\"font-size:14px;color: #000;padding:5px 8.5px 5px 5px;border: 2px solid #ccc;box-shadow: 2px 2px 0px #808080;-moz-border-radius: 3;-webkit-border-radius: 3px;border-radius:3px;px;background-color:#F7F7F7; clear:left;float:left;text-align:center;position:absolute;"
        if (This.ShowType == 2) TempletStr += "writing-mode: tb-rl;";
        TempletStr += "\" title=\"{Description}\" ><a style=\"color: #000;text-decoration: none;white-space: nowrap;\" href='{Link}' target=\"_blank\">{Text}</a></div>";
        //TempletStr += "\" title=\"{Description}\" ><a style=\"color: #666666;text-decoration: none;display:block;width:" + boxwidth + "px;\" target=\"_blank\">{Text}</a></div>";无滚动条时控制a标签宽度 
        return TempletStr;
    }

    function CreateLine(type_, top_, left_, long_, size_, color_, id_,targetdiv_) {
        this.Type = type_;
        top_ = top_ + "";
        left_ = left_ + "";
        long_ = long_ + "";
        ...
        this.Color = (color_ == undefined) ? "#ccc" : color_;
        this.TargetDivId=targetdiv_;
        this.Line = document.createElement("DIV");
        //document.body.appendChild(this.Line);
        document.getElementById(this.TargetDivId).appendChild(this.Line);
