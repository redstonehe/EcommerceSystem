    ...
    public partial class OrderController : BaseMobileController
    {
        ...
        public ActionResult ConfirmOrder()
        {
            ...
            foreach (StoreCartInfo item in storeCartList)
            {
                ...
                model.AllHonBaoCutFee += Carts.SumOrderProductHongBaoCutAmount(item.SelectedOrderProductList);
                model.TaxFee += Carts.SumOrderProductTaxAmount(item.SelectedOrderProductList);
                model.AllProductAmount += Carts.SumOrderProductAmount(item.SelectedOrderProductList);
                model.AllFullCut += Carts.SumFullCut(item.CartItemList);
        ...
        public ActionResult ConfirmOrder_DirectBuy()
        {
            ...
            string payName = WebHelper.GetFormString("payName");//"custompay";// ;//支付插件名称
        ...
        public ActionResult SubmitOrder()
        {
            lock (_locker)
            {
                ...
                if (haiMiCount > 0)
                {
                    ...
                    {
                        return AjaxResult("nohaimiaccount", "您的" + MallKey.MallDiscountName_JiangJin + "账号余额已被锁定。");
                    }
                    AccountInfo accontInfo = accountInfoList.Find(x => x.AccountId == (int)AccountType.商城钱包);
                    if (accontInfo == null)
                        return AjaxResult("nohaimiaccount", "你还没有拥有" + MallKey.MallDiscountName_JiangJin + "账号，请联系客服");

                    if (haiMiCount > accontInfo.Banlance)
                        return AjaxResult("noenoughhaimi", "你使用的" + MallKey.MallDiscountName_JiangJin + "数超过你所拥有的" + accontInfo.Banlance + "数");
                }
                //验证红包
                ...
                if (hongBaoCount > 0)
                {
                    ...
                    if (accontInfo == null)
                        return AjaxResult("nohongbaoaccount", "你还没有拥有" + MallKey.MallDiscountName_JiFen + "账号，请联系客服");

                    if (hongBaoCount > accontInfo.Banlance)
                        return AjaxResult("noenoughhongbao", "你使用的" + MallKey.MallDiscountName_JiFen + "超过你所拥有的" + accontInfo.Banlance + "数");
                }
                //验证汇购卡券
                ...
                foreach (StoreCartInfo item in storeCartList)
                {
                    ...
                    storeOrder.ShipFee = Orders.GetDoublee11ShipFee(item.SelectedOrderProductList, storeOrder.ShipFee, gpInfo);
                    ...
                    decimal currentShipFee = Orders.GetShipFee(fullShipAddressInfo.ProvinceId, fullShipAddressInfo.CityId, item.SelectedOrderProductList, ref isSend);// model.DefaultFullShipAddressInfo != null ? Orders.GetShipFee(item.StoreInfo.StoreId, storeOrder.ProductAmount) : 0;// 
                    decimal oldShipFee = currentShipFee;
                    currentShipFee = Orders.GetDoublee11ShipFee(item.SelectedOrderProductList, oldShipFee, gpInfo);

                    model.AllShipFee += currentShipFee;
                    model.AllTotalCount += Carts.SumOrderProductCount(item.SelectedOrderProductList);
                    model.AllTotalWeight += Carts.SumOrderProductWeight(item.SelectedOrderProductList);
                    exorderproductlist.AddRange(item.SelectedOrderProductList);
                }
                ...
                foreach (StoreCartInfo storeCartInfo in storeCartList)
                {
                    ...
                    //如果是海之圣旗舰店并且为在货到付款方式 提交订单后自动确认 确认后发送订单信息到直销系统
        ...
        public ActionResult SubmitResult()
        {
            ...
            foreach (string oid in StringHelper.SplitString(oidList))
            {
                ...
                if (orderInfo != null && (orderInfo.Uid == WorkContext.Uid || orderInfo.Uid == 0) && (orderInfo.OrderState == (int)OrderState.WaitPaying || orderInfo.OrderState == (int)OrderState.Confirmed) && (paySystemName.Length == 0 || paySystemName == orderInfo.PaySystemName))
                    ...
                    return Redirect(Url.Action("orderlist", "ucenter"));
        ...
        public ActionResult ResultPay(string oids, string paystatus)
        {
            ...
            //if (HttpContext.Request.UrlReferrer.ToString().Contains("/agent")) { }
            //if (ControllerContext.RouteData.DataTokens["area"].ToString().Contains("agent")) {
            //    return RedirectToAction("ResultPay", "Order", new { oids = oids, paystatus = paystatus, area = "agent" });
            //}
            ...
            foreach (string oid in StringHelper.SplitString(oids, ","))
            {
                //订单信息
                OrderInfo orderInfo = Orders.GetOrderByOid(TypeHelper.StringToInt(oid));
                //if (orderInfo != null && orderInfo.Uid == WorkContext.Uid && (orderInfo.OrderState == (int)OrderState.Confirmed || orderInfo.OrderState == (int)OrderState.Completed))
                //    successOrderList.Add(orderInfo);
        ...
        public ActionResult TestPay(string oids, string paystatus)
        {
            ...
            foreach (string oid in StringHelper.SplitString(oids, ","))
            {
                ...
                //VMall.Core.LogHelper.WriteOperateLog("TestLKLPay", "测试拉卡拉支付后台修改状态记录", "修改信息为：订单id：" + oid + "交易号:11111111111111" + "交易状态" + OrderState.Confirmed);
                OrderActions.CreateOrderAction(new OrderActionInfo()
                {
        ...
        /// <param name="paystatus"></param>
        /// <returns></returns>
        public ActionResult TestPay2(string oids, string paystatus)
        {
            PayResultModel payResuleModel = new PayResultModel();
            List<OrderInfo> successOrderList = new List<OrderInfo>();
            List<OrderInfo> failOrderList = new List<OrderInfo>();
            foreach (string oid in StringHelper.SplitString(oids, ","))
            {
                ...
                if (orderInfo != null && orderInfo.Uid == WorkContext.Uid && orderInfo.OrderState == (int)OrderState.Confirmed && orderInfo.OrderState <= (int)OrderState.Completed && (orderInfo.StoreId == WebHelper.GetConfigSettingsInt("AgentStoreId") || orderInfo.StoreId == WebHelper.GetConfigSettingsInt("AgentSuitStoreId")))
                    successOrderList.Add(orderInfo);
            ...
            payResuleModel.FailOrderList = failOrderList;
            payResuleModel.State = paystatus == "0" ? -1 : (paystatus == "1" ? 1 : 0);
            return RedirectToAction("ResultPay", "Order", new { oids = oids, paystatus = paystatus });
        }
        ...
        protected sealed override void OnAuthorization(AuthorizationContext filterContext)
        {
            ...
            //不允许游客访问
            //if (WorkContext.Uid < 1 && WorkContext.Action != "confirmorder_directbuy" && WorkContext.Action != "submitorder_directbuy" && WorkContext.Action != "submitresult" && WorkContext.Action != "payshow" && WorkContext.Action != "payresult" && WorkContext.Action != "resultpay" && WorkContext.Action != "testpay")
