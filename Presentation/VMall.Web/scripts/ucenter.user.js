addShipAddressResponse//删除收藏夹中的商品
...
function editUser() {
    ...
    var BankName = userInfoForm.elements["BankName"].value;
    var BankCardCode = userInfoForm.elements["BankCardCode"].value;
    var BankUserName = userInfoForm.elements["BankUserName"].value;
    if (!verifyEditUser(userName, nickName, realName, address, bio)) {
        return;
    }

    Ajax.post("/ucenter/edituser",
            { 'userName': userName, 'nickName': nickName, 'realName': realName, 'avatar': avatar, 'gender': gender, 'idCard': idCard, 'bday': bday, 'regionId': regionId, 'address': address, 'bio': bio, 'BankName': BankName, 'BankCardCode': BankCardCode, 'BankUserName': BankUserName },
            false,
            editUserResponse)
}
