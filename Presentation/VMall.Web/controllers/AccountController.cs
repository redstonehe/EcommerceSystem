...
namespace VMall.Web.Controllers
{
    ...
    public partial class AccountController : BaseWebController
    {
        ...
        {
            string email = "583557971@qq.com";
            bool flag = Emails.SendWebcomeEmail(email);
        ...
        /// </summary>
        /// <param name="mobile"></param>
        /// <param name="VcodeCall"></param>
        /// <returns></returns>
        public ActionResult SendCallMoblieVerify(string mobile, string VcodeCall)
        {
            //验证验证码

            if (string.IsNullOrWhiteSpace(VcodeCall))
            {
                return AjaxResult("fail", "系统繁忙");
            }
            else if (VcodeCall.ToLower() != Sessions.GetValueString(WorkContext.Sid, "VerifyCodeForRegisterCall"))
            {
                return AjaxResult("fail", "系统繁忙");
            }
            if (!LimitHelper.Execution(WorkContext.IP, (int)LimitEnum.RegisterForIP, 60 * 24, 10, true))
            ...
            //if (SMSes.SendRegisterSMS(mobile, verifyValue)) {
            if (SMSes.SendMobileVoiceMessage(verifyValue, mobile))
            {
                //将验证值保存到session中
                Sessions.SetItem(sid, "resigterMobile", mobile);
                Sessions.SetItem(sid, "mobileVerifyCode", verifyValue);
        ...
        public bool CheckGeeTestResult()
        {
            ...
            //else Response.Write(" ");
            //if (result == 1) 
            //    return Content("<div id='sb'>success</div>");
            //else
            //    return Content(" ");
        ...
        public ActionResult Login()
        {
            ...
            else//验证成功时
            {
                ...
                //将用户信息写入cookie中
                MallUtils.SetUserCookie(partUserInfo, (WorkContext.MallConfig.IsRemember == 1 && isRemember == 1) ? 1 : -1);
                ...
                //密码写入session
                //Session[partUserInfo.Uid + "_enpw"] = GetPasswordCookie(password);
        ...
        //                errorList.AppendFormat("{0}\"key\":\"{1}\",\"msg\":\"{2}\"{3},", "{", "accountName", "不能使用'" + emailProvider + "'类型的邮箱", "}");
        //            }
        //            else if (Users.IsExistEmail(accountName) || AccountUtils.IsUserExistForDirSale(accountName))
        //            {
        ...
        //        {
        //            if (VMall.Services.Users.IsExistUserName(accountName) || AccountUtils.IsUserExistForDirSale(accountName))
        //            {
        ...
        //        #region 处理用户注册推荐人关系
        //        //（取得地址栏中的推荐人并判断推荐人是否合法，不合法则系统分配一个默认推荐人-取地址最相近的用户为新注册用户推荐人）
        ...
        /// <returns></returns>
        //public bool AccountUtils.IsUserExistForDirSale(string accountName)
        //{
        ...
        /// <param name="pName"></param>
        /// <returns></returns>
        //public string GetUserNameByCodeForDir(string pName)
        //{
        //    //http://www.xxx.com/api/User/GetUserNameByCode?userCode=xxx
        //    string showname = "";
