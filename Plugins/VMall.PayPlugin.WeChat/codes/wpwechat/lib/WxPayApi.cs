...
namespace VMall.PayPlugin.WeChat
{
    public class WxPayApi
    {
        ...
        */
        public static WxPayData Refund(WxPayData inputObj, int timeOut = 10)
        {
            ...
            LogHelper.WriteOperateLog("WeChatRefund", "微信接口记录", "WxPayApi Refund request : " + xml);
            string response = HttpService.Post(xml, url, true, timeOut);//调用HTTP通信接口提交数据到API
            //Log.Debug("WxPayApi", "Refund response : " + response);
            LogHelper.WriteOperateLog("WeChatRefund", "微信接口记录", "WxPayApi Refund response : " + response);
            var end = DateTime.Now;
            int timeCost = (int)((end - start).TotalMilliseconds);//获得接口耗时
        ...
        */
        ...
        {
            //Log.Info("", "============ 调用统一下单接口开始 =====================");
            //LogHelper.WriteOperateLog("WeChatPayUnifiedOrder", "微信接口记录", "============ 调用统一下单接口开始 =====================");
            ...
            var start = DateTime.Now;
            //Log.Info("", "============ 调用统一下单接口请求参数 =====================");
            //LogHelper.WriteOperateLog("WeChatPayUnifiedOrder", "微信支付记录", "============ 调用统一下单接口请求参数 =====================");
            //Log.Debug("WxPayApi", "UnfiedOrder request : " + xml);
            //LogHelper.WriteOperateLog("WeChatPayUnifiedOrder", "微信接口记录", "WxPayApi UnfiedOrder request : " + xml);
            string response = HttpService.Post(xml, url, false, timeOut);
            //Log.Info("", "============ 调用统一下单接口返回参数 =====================");
            //Log.Debug("WxPayApi", "UnfiedOrder response : " + response);
            //LogHelper.WriteOperateLog("WeChatPayUnifiedOrder", "微信接口记录", "WxPayApi UnfiedOrder response : " + response);
            var end = DateTime.Now;
            int timeCost = (int)((end - start).TotalMilliseconds);
           ...
           //ReportCostTime(url, timeCost, result);//测速上报
            ...
            //Log.Info("", "============ 调用统一下单接口结束 =====================");
            //LogHelper.WriteOperateLog("WeChatPayUnifiedOrder", "微信接口记录", "============ 调用统一下单接口结束 =====================");
            return result;
        }
