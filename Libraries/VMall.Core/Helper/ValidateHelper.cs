...
namespace VMall.Core
{
    ...
    public class ValidateHelper
    {
        ...
        //手机号正则表达式
        private static Regex _mobileregex = new Regex("^(13|14|15|17|18|19)[0-9]{9}$");
        //固话号正则表达式
        private static Regex _phoneregex = new Regex(@"^(\d{3,4}-?)?\d{7,8}|((400|800)([0-9\\-]{7,10}))$");
        //IP正则表达式
        ...
        /// </summary>
        /// <returns> </returns>
        public static bool IsExcelFileName(string fileName)
        {
            if (fileName.IndexOf(".") == -1)
                return false;
