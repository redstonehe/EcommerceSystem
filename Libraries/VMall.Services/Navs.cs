...
namespace VMall.Services
{
    ...
    public partial class Navs
    {
        ...
        {
            List<NavInfo> navTree = VMall.Core.BMACache.Get(CacheKeys.MALL_NAV_LIST) as List<NavInfo>;
            ...
            if (navTree == null)
            {
                ...
                VMall.Core.BMACache.Insert(CacheKeys.MALL_NAV_LIST, navTree);
            }
            return navTree;
        }
        ...
        {
            List<NavInfo> navList = VMall.Core.BMACache.Get(CacheKeys.MALL_NAV_MAINLIST) as List<NavInfo>;

            if (navList == null)
            {
                navList = new List<NavInfo>();
                foreach (NavInfo navInfo in VMall.Data.Navs.GetNavList())
                {
                    if (navInfo.Pid == 0)
                        navList.Add(navInfo);
                }
                VMall.Core.BMACache.Insert(CacheKeys.MALL_NAV_MAINLIST, navList);
            }
            return navList;
        }
