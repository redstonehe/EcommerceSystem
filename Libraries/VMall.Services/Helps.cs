...
namespace VMall.Services
{
    ...
    public partial class Helps
    {
        ...
        {
            List<HelpInfo> helpTree = VMall.Core.BMACache.Get(CacheKeys.MALL_HELP_LIST) as List<HelpInfo>;

            if (helpTree == null)
            {
                helpTree = new List<HelpInfo>();
                List<HelpInfo> helpList = VMall.Data.Helps.GetHelpList();

                CreateHelpTree(helpList, helpTree, 0);
                VMall.Core.BMACache.Insert(CacheKeys.MALL_HELP_LIST, helpTree);
            }
            return helpTree;
        }
