...
namespace VMall.RDBSStrategy.SqlServer
{
    ...
    public partial class RDBSStrategy : IRDBSStrategy
    {
        ...
        public DataTable AdminGetStoreList(int pageSize, int pageNumber, string condition, string sort)
        {
            ...
            {
                if (noCondition)
                    commandText = string.Format(@" SELECT  [storeid],[state],[name],[storerid],[createtime],[mallsource]  FROM dbo.hlh_stores s ORDER BY storeid ASC",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre,
                                                sort);

                else
                    commandText = string.Format("SELECT [temp1].[storeid],[temp1].[state],[temp1].[name],[temp1].[storerid],[temp1].[createtime],[mallsource],[temp2].[title] FROM (SELECT TOP {0} [storeid],[state],[name],[storerid],[createtime],[mallsource] FROM [{1}stores] WHERE {2} ORDER BY {3}) AS [temp1] LEFT JOIN [{1}storeranks] AS [temp2] ON [temp1].[storerid]=[temp2].[storerid] ",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre,
                                                condition,
                                                sort);
            ...
            {
                if (noCondition)
                    commandText = string.Format("SELECT [temp1].[storeid],[temp1].[state],[temp1].[name],[temp1].[storerid],[temp1].[createtime],[mallsource],[temp2].[title] FROM (SELECT TOP {0} [storeid],[state],[name],[storerid],[createtime],[mallsource] FROM [{1}stores] WHERE [storeid] NOT IN (SELECT TOP {2} [storeid] FROM [{1}stores] ORDER BY {3}) ORDER BY {3}) AS [temp1] LEFT JOIN [{1}storeranks] AS [temp2] ON [temp1].[storerid]=[temp2].[storerid] ",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre,
                                                (pageNumber - 1) * pageSize,
                                                sort);
                else
                    commandText = string.Format("SELECT [temp1].[storeid],[temp1].[state],[temp1].[name],[temp1].[storerid],[temp1].[createtime],[mallsource],[temp2].[title] FROM (SELECT TOP {0} [storeid],[state],[name],[storerid],[createtime],[mallsource] FROM [{1}stores] WHERE [storeid] NOT IN (SELECT TOP {2} [storeid] FROM [{1}stores] WHERE {3} ORDER BY {4}) AND {3} ORDER BY {4}) AS [temp1] LEFT JOIN [{1}storeranks] AS [temp2] ON [temp1].[storerid]=[temp2].[storerid] ",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre,
                                                (pageNumber - 1) * pageSize,
                                                condition,
        ...
        /// <returns></returns>
        public string AdminGetStoreListCondition(string storeName, int storeRid, int storeIid, int state,int storeId=0)
        {
            ...
            condition.Append(" AND [mallsource] in (0,2,3) ");
        ...
        public int CreateStoreShipTemplate(StoreShipTemplateInfo storeShipTemplateInfo)
        {
            DbParameter[] parms = {
                                        ...
                                        GenerateInParam("@freestartprice", SqlDbType.Decimal, 8, storeShipTemplateInfo.FreeStartPrice),
                                        GenerateInParam("@templateremark", SqlDbType.NVarChar, 500, storeShipTemplateInfo.TemplateRemark)
                                    };
            string commandText = string.Format("INSERT INTO [{0}storeshiptemplates]([storeid],[free],[type],[title],[freestartprice],[templateremark]) VALUES(@storeid,@free,@type,@title,@freestartprice,@templateremark);SELECT SCOPE_IDENTITY();",
                                                RDBSHelper.RDBSTablePre);
            return TypeHelper.ObjectToInt(RDBSHelper.ExecuteScalar(CommandType.Text, commandText, parms));
        }
        ...
        public void UpdateStoreShipTemplate(StoreShipTemplateInfo storeShipTemplateInfo)
        {
            DbParameter[] parms = {
                                        ...
                                        GenerateInParam("@freestartprice", SqlDbType.Decimal, 8, storeShipTemplateInfo.FreeStartPrice),
                                        GenerateInParam("@templateremark", SqlDbType.NVarChar, 500, storeShipTemplateInfo.TemplateRemark),
                                        GenerateInParam("@nosendarea", SqlDbType.NVarChar, 300, storeShipTemplateInfo.NoSendArea),
                                        GenerateInParam("@nosendcity", SqlDbType.NVarChar, 500, storeShipTemplateInfo.NoSendCity),
                                        GenerateInParam("@storestid", SqlDbType.Int,4, storeShipTemplateInfo.StoreSTid)
                                    };
            string commandText = string.Format("UPDATE [{0}storeshiptemplates] SET [storeid]=@storeid,[free]=@free,[type]=@type,[title]=@title,[freestartprice]=@freestartprice,[templateremark]=@templateremark,[nosendarea]=@nosendarea,[nosendcity]=@nosendcity WHERE [storestid]=@storestid",
                                                RDBSHelper.RDBSTablePre);
            RDBSHelper.ExecuteNonQuery(CommandType.Text, commandText, parms);
        }
        ...
        public IDataReader GetStoreShipTemplateById(int storeSTid)
        {
            ...
            string commandText = string.Format("SELECT * FROM [{0}storeshiptemplates] WHERE [storestid]=@storestid",RDBSHelper.RDBSTablePre);
            return RDBSHelper.ExecuteReader(CommandType.Text,commandText, parms);
        }
        ...
        public IDataReader GetStoreShipTemplateList(int storeId)
        {
            ...
            string commandText = string.Format("SELECT * FROM [{0}storeshiptemplates] WHERE [storeid]=@storeid",RDBSHelper.RDBSTablePre);
            return RDBSHelper.ExecuteReader(CommandType.Text, commandText, parms);
        }
        ...
        public void CreateStoreShipFee(StoreShipFeeInfo storeShipFeeInfo)
        {
            DbParameter[] parms = {
                                        ...
                                        GenerateInParam("@regionid", SqlDbType.VarChar,500, storeShipFeeInfo.RegionId),
                                        GenerateInParam("@startvalue", SqlDbType.Float,4, storeShipFeeInfo.StartValue),
                                        GenerateInParam("@startfee", SqlDbType.Decimal,8, storeShipFeeInfo.StartFee),
                                        GenerateInParam("@addvalue", SqlDbType.Float,4, storeShipFeeInfo.AddValue),
                                        GenerateInParam("@addfee", SqlDbType.Decimal,8, storeShipFeeInfo.AddFee),
                                        GenerateInParam("@shiptype", SqlDbType.TinyInt,1, storeShipFeeInfo.ShipType),
                                        GenerateInParam("@cityid", SqlDbType.VarChar,500, storeShipFeeInfo.CityId)
                                    };
            string commandText = string.Format("INSERT INTO [{0}storeshipfees]([storestid],[regionid],[startvalue],[startfee],[addvalue],[addfee],[shiptype],[cityid]) VALUES(@storestid,@regionid,@startvalue,@startfee,@addvalue,@addfee,@shiptype,@cityid)",
                                                RDBSHelper.RDBSTablePre);
            RDBSHelper.ExecuteNonQuery(CommandType.Text, commandText, parms);
        }
        ...
        public void UpdateStoreShipFee(StoreShipFeeInfo storeShipFeeInfo)
        {
            ...
            string commandText = string.Format("UPDATE [{0}storeshipfees] SET [storestid]=@storestid,[regionid]=@regionid,[startvalue]=@startvalue,[startfee]=@startfee,[addvalue]=@addvalue,[addfee]=@addfee,[shiptype]=@shiptype,[cityid]=@cityid WHERE [recordid]=@recordid",
                                                RDBSHelper.RDBSTablePre);
            RDBSHelper.ExecuteNonQuery(CommandType.Text, commandText, parms);
        }
        ...
        public IDataReader GetStoreShipFeeById(int recordId)
        {
            ...
            string commandText = string.Format("SELECT * FROM [{0}storeshipfees] WHERE [recordid]=@recordId",RDBSHelper.RDBSTablePre);
            return RDBSHelper.ExecuteReader(CommandType.Text, commandText, parms);
        }
        ...
        public IDataReader GetStoreShipFeeByStoreSTidAndRegion(int storeSTid, int provinceId, int cityId)
        {
            ...
            return RDBSHelper.ExecuteReader(CommandType.StoredProcedure,string.Format("{0}getstoreshipfeebystorestidandregion", RDBSHelper.RDBSTablePre),parms);
        }
