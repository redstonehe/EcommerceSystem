...
namespace VMall.RDBSStrategy.SqlServer
{
    ...
    public partial class RDBSStrategy : IRDBSStrategy
    {
        ...
        /// <param name="value">值</param>
        /// <returns></returns>
        private DbParameter GenerateParam(string paramName, SqlDbType sqlDbType, int size,byte round, ParameterDirection direction, object value)
        {
            SqlParameter param = new SqlParameter(paramName, sqlDbType, size, direction, false, (byte)5, round, "", DataRowVersion.Current, value);
            param.Direction = direction;
            param.Scale = round;
            if (direction == ParameterDirection.Input && value != null)
                param.Value = value;
