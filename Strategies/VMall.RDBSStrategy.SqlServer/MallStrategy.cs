...
namespace VMall.RDBSStrategy.SqlServer
{
    ...
    public partial class RDBSStrategy : IRDBSStrategy
    {
        ...
        {
            string commandText = string.Format("SELECT [temp3].[provinceid],[temp4].[name] AS [provincename],[temp3].[totalcount] FROM (SELECT [temp2].[parentid] AS [provinceid],SUM([temp1].[count]) AS [totalcount] FROM (SELECT [category],[value],[count] FROM [{0}pvstats] WHERE [category]='region' AND CONVERT(INT,value)>=0  ) AS [temp1] LEFT JOIN [{0}regions] AS [temp2] ON [temp1].[value]=[temp2].[regionid] GROUP BY [temp2].[parentid]) AS [temp3] LEFT JOIN [{0}regions] AS [temp4] ON [temp4].[regionid]=[temp3].[provinceid]",
                                                RDBSHelper.RDBSTablePre);
        ...
        public DataTable GetHomeBannerList(int type, DateTime nowTime)
        {
            ...
            string commandText = string.Format("SELECT * FROM [{0}banners] WHERE [type]=@type AND [starttime]<=@nowtime AND [endtime]>@nowtime AND [isshow]=1 ORDER BY [displayorder] DESC,[id] DESC",
                                                RDBSHelper.RDBSTablePre);
        ...
        /// <param name="pageNumber">当前页数</param>
        /// <returns></returns>
        public IDataReader AdminGetBannerList(int pageSize, int pageNumber, string title, int type)
        {
            string strWhere = " WHERE 1=1 AND type<100 ";
            string strWhere2 = string.Empty;
            string strWhere3 = string.Empty;
            if (!string.IsNullOrEmpty(title))
                strWhere2 = string.Format(" AND [title] like '%{0}%' ", title);
            if (type > -1)
                strWhere3 = string.Format(" AND [type]={0} ", type);
            string commandText;
            if (pageNumber == 1)
                commandText = string.Format("SELECT TOP {0} * FROM [{1}banners] {2} {3} {4} ORDER BY [type] ASC,[displayorder] DESC,[id] DESC",
                                            pageSize,
                                            RDBSHelper.RDBSTablePre,
                                            strWhere,strWhere2,strWhere3);

            else
                commandText = string.Format("SELECT TOP {0} * FROM [{1}banners] WHERE [id] NOT IN (SELECT TOP {2} [id] FROM [{1}banners] {3} {4} {5} ORDER BY [type] ASC,[displayorder] DESC,[id] DESC) ORDER BY [type] ASC,[displayorder] DESC,[id] DESC",
                                            pageSize,
                                            RDBSHelper.RDBSTablePre,
                                            (pageNumber - 1) * pageSize,
                                            strWhere, strWhere2, strWhere3);
        ...
        /// </summary>
        /// <returns></returns>
        public int AdminGetBannerCount(string title,int type)
        {
            string strWhere = " WHERE 1=1 AND type<100 ";
            string strWhere2 = string.Empty;
            string strWhere3 = string.Empty;
            ...
            if(type>-1)
                strWhere3 = string.Format(" AND [type]={0} ", type);
            string commandText = string.Format("SELECT COUNT(id) FROM [{0}banners] {1} {2} {3}", RDBSHelper.RDBSTablePre,strWhere,strWhere2,strWhere3);
            return TypeHelper.ObjectToInt(RDBSHelper.ExecuteScalar(CommandType.Text, commandText));
        }
        ...
        public IDataReader AdminGetBannerById(int id)
        {
            ...
            string commandText = String.Format("SELECT * FROM [{0}banners] WHERE [id]=@id",
                                                RDBSHelper.RDBSTablePre);
        ...
        public void CreateBanner(BannerInfo bannerInfo)
        {
            DbParameter[] param = {
                                    ...
                                    GenerateInParam("@extfield1",SqlDbType.NVarChar, 500, bannerInfo.ExtField1),
                                    GenerateInParam("@extfield2",SqlDbType.NVarChar, 500, bannerInfo.ExtField2),
                                    GenerateInParam("@extfield3",SqlDbType.NVarChar, 500, bannerInfo.ExtField3)
                                   };
            string commandText = String.Format("INSERT INTO [{0}banners]([type],[starttime],[endtime],[isshow],[title],[img],[url],[displayorder],[extfield1],[extfield2],[extfield3]) VALUES(@type,@starttime,@endtime,@isshow,@title,@img,@url,@displayorder,@extfield1,@extfield2,@extfield3)",
                                                RDBSHelper.RDBSTablePre);
            RDBSHelper.ExecuteNonQuery(CommandType.Text, commandText, param);
        }
        ...
        public void UpdateBanner(BannerInfo bannerInfo)
        {
            ...
            string commandText = String.Format("UPDATE [{0}banners] SET [type]=@type,[starttime]=@starttime,[endtime]=@endtime,[isshow]=@isshow,[title]=@title,[img]=@img,[url]=@url,[displayorder]=@displayorder,[extfield1]=@extfield1,[extfield2]=@extfield2,[extfield3]=@extfield3 WHERE [id]=@id",
                                                RDBSHelper.RDBSTablePre);
            RDBSHelper.ExecuteNonQuery(CommandType.Text, commandText, param);
        }
        ...
        /// <param name="pageNumber">当前页数</param>
        /// <returns></returns>
        public IDataReader GetAdvertPositionList(int pageSize, int pageNumber,string title)
        {
            string strWhere = " WHERE [mallsource]=0 ";
            if (!string.IsNullOrEmpty(title))
                strWhere += string.Format(" AND [title] like '%{0}%' ", title);
            string commandText;
            if (pageNumber == 1)
            {
                commandText = string.Format("SELECT TOP {2} {1} FROM [{0}advertpositions] {3} ORDER BY [adposid] DESC",
                                             RDBSHelper.RDBSTablePre,
            ...
            {
                commandText = string.Format("SELECT TOP {2} {1} FROM [{0}advertpositions] WHERE [adposid] < (SELECT MIN([adposid]) FROM (SELECT TOP {3} [adposid] FROM [{0}advertpositions] {4} ORDER BY [adposid] DESC) AS [temp]) ORDER BY [adposid] DESC",
                                             RDBSHelper.RDBSTablePre,
                                             RDBSFields.ADVERT_POSITIONS,
                                             pageSize,
                                             (pageNumber - 1) * pageSize,strWhere);
            }
        ...
        /// </summary>
        /// <returns></returns>
        public int GetAdvertPositionCount(string title)
        {
            string strWhere = " WHERE [mallsource]=0 ";
            if (!string.IsNullOrEmpty(title))
                strWhere += string.Format(" AND [title] like '%{0}%' ", title);
            string commandText = string.Format("SELECT COUNT(adposid) FROM [{0}advertpositions] {1}", RDBSHelper.RDBSTablePre, strWhere);
        ...
        {
            string commandText = string.Format("SELECT {1} FROM [{0}advertpositions] WHERE [mallsource]=0  ORDER BY [adposid] DESC",
                                                RDBSHelper.RDBSTablePre,
                                                RDBSFields.ADVERT_POSITIONS);
            return RDBSHelper.ExecuteReader(CommandType.Text, commandText);
        }
        ...
        /// <param name="adPosId">广告位置id</param>
        /// <returns></returns>
        public DataTable AdminGetAdvertList(int pageSize, int pageNumber, int adPosId,string title)
        {
            string strWhere = string.Empty;
            if (!string.IsNullOrEmpty(title))
                if (adPosId < 1)
                    strWhere = string.Format(" WHERE [title] like '%{0}%' AND  adposid in (select adposid from hlh_advertpositions where mallsource=0) ", title);
                else
                    strWhere = string.Format(" AND [title] like '%{0}%' AND  adposid in (select adposid from hlh_advertpositions where mallsource=0) ", title);
            else
                if (adPosId < 1)
                    strWhere = string.Format(" WHERE   adposid in (select adposid from hlh_advertpositions where mallsource=0) ", title);
                else
                    strWhere = string.Format("  AND  adposid in (select adposid from hlh_advertpositions where mallsource=0) ", title);
            string commandText;
            if (pageNumber == 1)
            {
                if (adPosId < 1)
                    commandText = string.Format("SELECT [temp1].[adid],[temp1].[clickcount],[temp1].[adposid],[temp1].[type],[temp1].[title] AS [atitle],[temp1].[starttime],[temp1].[endtime],[temp1].[state],[temp1].[displayorder],[temp2].[title] AS [aptitle] FROM (SELECT TOP {0} [adid],[clickcount],[adposid],[type],[title],[starttime],[endtime],[state],[displayorder] FROM [{1}adverts] {2} ORDER BY [adposid] DESC,[displayorder] DESC,[adid] DESC) AS [temp1] LEFT JOIN [{1}advertpositions] AS [temp2] ON [temp1].[adposid]=[temp2].[adposid]",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre, 
                                                strWhere);

                else
                    commandText = string.Format("SELECT [temp1].[adid],[temp1].[clickcount],[temp1].[adposid],[temp1].[type],[temp1].[title] AS [atitle],[temp1].[starttime],[temp1].[endtime],[temp1].[state],[temp1].[displayorder],[temp2].[title] AS [aptitle] FROM (SELECT TOP {0} [adid],[clickcount],[adposid],[type],[title],[starttime],[endtime],[state],[displayorder] FROM [{1}adverts] WHERE [adposid]={2} {3} ORDER BY [adposid] DESC,[displayorder] DESC,[adid] DESC) AS [temp1] LEFT JOIN [{1}advertpositions] AS [temp2] ON [temp1].[adposid]=[temp2].[adposid]",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre,
                                                adPosId,
                                                strWhere);
            ...
            {
                if (adPosId < 1)
                    commandText = string.Format("SELECT [temp1].[adid],[temp1].[clickcount],[temp1].[adposid],[temp1].[type],[temp1].[title] AS [atitle],[temp1].[starttime],[temp1].[endtime],[temp1].[state],[temp1].[displayorder],[temp2].[title] AS [aptitle] FROM (SELECT TOP {0} [adid],[clickcount],[adposid],[type],[title],[starttime],[endtime],[state],[displayorder] FROM [{1}adverts] WHERE [adid] NOT IN (SELECT TOP {2} [adid] FROM [{1}adverts] {3} ORDER BY [adposid] DESC,[displayorder] DESC,[adid] DESC) ORDER BY [adposid] DESC,[displayorder] DESC,[adid] DESC) AS [temp1] LEFT JOIN [{1}advertpositions] AS [temp2] ON [temp1].[adposid]=[temp2].[adposid]",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre,
                                                (pageNumber - 1) * pageSize,
                                                strWhere);
                else
                    commandText = string.Format("SELECT [temp1].[adid],[temp1].[clickcount],[temp1].[adposid],[temp1].[type],[temp1].[title] AS [atitle],[temp1].[starttime],[temp1].[endtime],[temp1].[state],[temp1].[displayorder],[temp2].[title] AS [aptitle] FROM (SELECT TOP {0} [adid],[clickcount],[adposid],[type],[title],[starttime],[endtime],[state],[displayorder] FROM [{1}adverts] WHERE [adposid]={3} AND [adid] NOT IN (SELECT TOP {2} [adid] FROM [{1}adverts] WHERE [adposid]={3} {4} ORDER BY [adposid] DESC,[displayorder] DESC,[adid] DESC) ORDER BY [adposid] DESC,[displayorder] DESC,[adid] DESC) AS [temp1] LEFT JOIN [{1}advertpositions] AS [temp2] ON [temp1].[adposid]=[temp2].[adposid]",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre,
                                                (pageNumber - 1) * pageSize,
                                                adPosId,
        ...
        /// <param name="adPosId">广告位置id</param>
        /// <returns></returns>
        public int AdminGetAdvertCount(int adPosId,string title)
        {
            string strWhere = string.Empty;
            if (!string.IsNullOrEmpty(title))
                if (adPosId < 1)
            ...
            string commandText;
            if (adPosId < 1)
                commandText = string.Format("SELECT COUNT(adid) FROM [{0}adverts] {1}", RDBSHelper.RDBSTablePre, strWhere);
            else
                commandText = string.Format("SELECT COUNT(adid) FROM [{0}adverts] WHERE [adposid]={1} {2}", RDBSHelper.RDBSTablePre, adPosId, strWhere);
        ...
        {
            string commandText = string.Format("SELECT {1} FROM [{0}newstypes] WHERE mallsource=0 ORDER BY [displayorder] DESC",
                                                RDBSHelper.RDBSTablePre,
        ...
        public string AdminGetNewsListCondition(int newsTypeId, string title)
        {
            ...
            condition.AppendFormat(" AND [mallsource] = {0} ", 0);
            return condition.Length > 0 ? condition.Remove(0, 4).ToString() : "";
        }
        ...
        {
            string commandText = string.Format("SELECT {1} FROM [{0}helps] WHERE mallsource=0 ORDER BY [displayorder] DESC",
                                                RDBSHelper.RDBSTablePre,
                                                RDBSFields.HELPS);
            return RDBSHelper.ExecuteReader(CommandType.Text, commandText);
        }
