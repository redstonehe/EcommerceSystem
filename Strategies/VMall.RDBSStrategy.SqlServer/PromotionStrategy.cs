...
namespace VMall.RDBSStrategy.SqlServer
{
    ...
    public partial class RDBSStrategy : IRDBSStrategy
    {
        ...
        public IDataReader AdminGetSinglePromotionById(int pmId)
        {
            ...
            string commandText = string.Format("SELECT * FROM [{0}singlepromotions] WHERE [pmid]=@pmid",RDBSHelper.RDBSTablePre);
        ...
        public void CreateSinglePromotion(SinglePromotionInfo singlePromotionInfo)
        {
            DbParameter[] parms = {
                                        ...
                                        GenerateInParam("@discountvalue", SqlDbType.Decimal,8,singlePromotionInfo.DiscountValue),
                                        ...
                                        GenerateInParam("@promohaimi", SqlDbType.Decimal,8,singlePromotionInfo.PromoHaiMi),
                                        GenerateInParam("@promopv", SqlDbType.Decimal,8,singlePromotionInfo.PromoPV),
                                        GenerateInParam("@promohongbaocut", SqlDbType.Decimal,8,singlePromotionInfo.PromoHongBaoCut),
                                        GenerateInParam("@isshow", SqlDbType.Int,4,singlePromotionInfo.IsShow),
                                        GenerateInParam("@amount1", SqlDbType.Decimal,8,singlePromotionInfo.Amount1),
                                        GenerateInParam("@discount1", SqlDbType.Decimal,8,singlePromotionInfo.Discount1),
                                        GenerateInParam("@amount2", SqlDbType.Decimal,8,singlePromotionInfo.Amount2),
                                        GenerateInParam("@discount2", SqlDbType.Decimal,8,singlePromotionInfo.Discount2),
                                        GenerateInParam("@amount3", SqlDbType.Decimal,8,singlePromotionInfo.Amount3),
                                        GenerateInParam("@discount3", SqlDbType.Decimal,8,singlePromotionInfo.Discount3),
                                        GenerateInParam("@amount4", SqlDbType.Decimal,8,singlePromotionInfo.Amount4),
                                        GenerateInParam("@discount4", SqlDbType.Decimal,8,singlePromotionInfo.Discount4),
                                        GenerateInParam("@amount5", SqlDbType.Decimal,8,singlePromotionInfo.Amount5),
                                        GenerateInParam("@discount5", SqlDbType.Decimal,8,singlePromotionInfo.Discount5),
                                    };
            string commandText = string.Format("INSERT INTO [{0}singlepromotions]([pid],[storeid],[starttime1],[endtime1],[starttime2],[endtime2],[starttime3],[endtime3],[userranklower],[state],[name],[slogan],[discounttype],[discountvalue],[coupontypeid],[paycredits],[isstock],[stock],[quotalower],[quotaupper],[allowbuycount],[promohaimi],[promopv],[promohongbaocut],[isshow],[amount1],[discount1],[amount2],[discount2],[amount3],[discount3],[amount4],[discount4],[amount5],[discount5]) VALUES(@pid,@storeid,@starttime1,@endtime1,@starttime2,@endtime2,@starttime3,@endtime3,@userranklower,@state,@name,@slogan,@discounttype,@discountvalue,@coupontypeid,@paycredits,@isstock,@stock,@quotalower,@quotaupper,@allowbuycount,@promohaimi,@promopv,@promohongbaocut,@isshow,@amount1,@discount1,@amount2,@discount2,@amount3,@discount3,@amount4,@discount4,@amount5,@discount5)",
        ...
        public DataTable AdminGetSinglePromotionList(int pageSize, int pageNumber, string condition, string sort)
        {
            ...
            if (pageNumber == 1)
            {
                if (noCondition)
                    ...
                    commandText = string.Format(" SELECT TOP {0} a.*,b.name AS pname,c.name AS sname FROM dbo.hlh_singlepromotions a LEFT JOIN dbo.hlh_products  b ON a.pid=b.pid LEFT JOIN dbo.hlh_stores  c ON a.storeid=c.storeid ORDER BY a.pmid DESC", pageSize);
                    ...
                    //                            sort,
                    //                            condition);
                    commandText = string.Format(" SELECT TOP {0} a.*,b.name AS pname,c.name AS sname FROM dbo.hlh_singlepromotions a LEFT JOIN dbo.hlh_products  b ON a.pid=b.pid LEFT JOIN dbo.hlh_stores  c ON a.storeid=c.storeid where {1} ORDER BY a.pmid DESC", pageSize, condition);
            }
            else
            {
                if (noCondition)
 ...
 SELECT ROW_NUMBER() OVER (ORDER BY [pmid] DESC ) AS [rowid],a.*,b.name AS pname,c.name AS sname FROM dbo.hlh_singlepromotions a LEFT JOIN dbo.hlh_products  b ON a.pid=b.pid LEFT JOIN dbo.hlh_stores  c ON a.storeid=c.storeid)
 select * from tb where rowid between (" + pageNumber + @"-1)*" + pageSize + @"+1 and (" + pageNumber + @")*" + pageSize + " order by tb.pmid desc",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre,
                    ...
                    //                            condition);
                commandText = string.Format(@" WITH tb as (
 SELECT ROW_NUMBER() OVER (ORDER BY [pmid] DESC ) AS [rowid],a.*,b.name AS pname,c.name AS sname FROM dbo.hlh_singlepromotions a LEFT JOIN dbo.hlh_products  b ON a.pid=b.pid LEFT JOIN dbo.hlh_stores  c ON a.storeid=c.storeid WHERE {0})
 select * from tb where rowid between (" + pageNumber + @"-1)*" + pageSize + @"+1 and (" + pageNumber + @")*" + pageSize + " order by tb.pmid desc",condition);
            }
        ...
        public string AdminGetSinglePromotionListCondition(int storeId, int pid, string promotionName, string promotionTime)
        {
            ...
            if (!string.IsNullOrWhiteSpace(promotionName))
                condition.AppendFormat(" AND a.[name] like '%{0}%'", promotionName);
        ...
        public int AdminGetSinglePromotionCount(string condition)
        {
            ...
            if (string.IsNullOrWhiteSpace(condition))
                commandText = string.Format("SELECT COUNT(pmid) FROM [{0}singlepromotions] a", RDBSHelper.RDBSTablePre);
            else
                commandText = string.Format("SELECT COUNT(pmid) FROM [{0}singlepromotions] a WHERE {1}", RDBSHelper.RDBSTablePre, condition);
        ...
        public IDataReader GetSinglePromotionByPidAndTime(int pid, DateTime nowTime)
        {
           ...
           // SELECT TOP 1 * FROM [hlh_singlepromotions]  WHERE [pid]=@pid  AND (([starttime1]<=@nowtime AND [endtime1]>@nowtime) OR ([starttime2]<=@nowtime AND [endtime2]>@nowtime) OR  ([starttime3]<=@nowtime AND [endtime3]>@nowtime)) AND [state]=1 

            string commandText = string.Format("SELECT TOP 1 * FROM [{0}singlepromotions]  WHERE [pid]=@pid  AND (([starttime1]<=@nowtime AND [endtime1]>@nowtime) OR ([starttime2]<=@nowtime AND [endtime2]>@nowtime) OR  ([starttime3]<=@nowtime AND [endtime3]>@nowtime)) AND [state]=1",RDBSHelper.RDBSTablePre);
            return RDBSHelper.ExecuteReader(CommandType.Text, commandText, parms);
            //return RDBSHelper.ExecuteReader(CommandType.StoredProcedure,string.Format("{0}getsinglepromotionbypidandtime", RDBSHelper.RDBSTablePre),parms);
        }
        ...
        public IDataReader AdminGetSuitPromotionById(int pmId)
        {
            ...
            string commandText = string.Format("SELECT * FROM [{0}suitpromotions] WHERE [pmid]=@pmid",
                                                RDBSHelper.RDBSTablePre,
        ...
        public int CreateSuitPromotion(SuitPromotionInfo suitPromotionInfo)
        {
            DbParameter[] parms = {
                                        ...
                                        GenerateInParam("@suitpv", SqlDbType.Decimal, 8, suitPromotionInfo.SuitPV),
                                        GenerateInParam("@suithaimi", SqlDbType.Decimal, 8, suitPromotionInfo.SuitHaiMi),
                                        GenerateInParam("@suithongbaocut", SqlDbType.Decimal, 8, suitPromotionInfo.SuitHongBaoCut)
                                    };
            string commandText = string.Format("INSERT INTO [{0}suitpromotions]([storeid],[starttime1],[endtime1],[starttime2],[endtime2],[starttime3],[endtime3],[userranklower],[state],[name],[quotaupper],[onlyonce],[suitpv],[suithaimi],[suithongbaocut]) VALUES(@storeid,@starttime1,@endtime1,@starttime2,@endtime2,@starttime3,@endtime3,@userranklower,@state,@name,@quotaupper,@onlyonce,@suitpv,@suithaimi,@suithongbaocut);SELECT SCOPE_IDENTITY();",
                                                RDBSHelper.RDBSTablePre);
        ...
        /// 添加套装商品
        /// </summary>
        public void AddSuitProduct(int pmId, int pid, decimal discount, int number)
        {
            DbParameter[] parms = {
                                        GenerateInParam("@pmid", SqlDbType.Int, 4, pmId),
                                        GenerateInParam("@pid", SqlDbType.Int,4,pid),
        ...
        /// <returns></returns>
        public bool UpdateSuitProductDiscount(int pmId, int pid, decimal discount)
        {
            DbParameter[] parms = {
                                        GenerateInParam("@pmid", SqlDbType.Int, 4, pmId),
                                        GenerateInParam("@pid", SqlDbType.Int,4,pid),
	...
	[temp2].[weight],
	[temp2].[showimg] 
	FROM (SELECT [recordid],[pmid],[pid],[discount],[number] FROM [hlh_suitproducts] WHERE [pmid] IN ({0})) AS [temp1] 
	LEFT JOIN [hlh_products] AS [temp2] 
	ON [temp1].[pid]=[temp2].[pid]
	WHERE [temp2].[state]=0
	ORDER BY [temp1].[pmid] DESC",pmIdList);
            return RDBSHelper.ExecuteReader(CommandType.Text, commandText);
            //return RDBSHelper.ExecuteReader(CommandType.StoredProcedure,string.Format("{0}getallextsuitproductlist", RDBSHelper.RDBSTablePre),parms);
        }
        ...
        public void CreateCouponType(CouponTypeInfo couponTypeInfo)
        {
            DbParameter[] parms = {
                                        ...
                                        GenerateInParam("@channelid", SqlDbType.VarChar,500,couponTypeInfo.ChannelId)
                                    };
            string commandText = string.Format("INSERT INTO [{0}coupontypes]([storeid],[state],[name],[money],[count],[sendmode],[getmode],[usemode],[userranklower],[orderamountlower],[limitstorecid],[limitproduct],[sendstarttime],[sendendtime],[useexpiretime],[usestarttime],[useendtime],[channelid]) VALUES(@storeid,@state,@name,@money,@count,@sendmode,@getmode,@usemode,@userranklower,@orderamountlower,@limitstorecid,@limitproduct,@sendstarttime,@sendendtime,@useexpiretime,@usestarttime,@useendtime,@channelid)",
                                                RDBSHelper.RDBSTablePre);
            RDBSHelper.ExecuteNonQuery(CommandType.Text, commandText, parms);
        }
        ...
        public IDataReader GetCouponTypeById(int couponTypeId)
        {
            ...
            string commandText = string.Format("SELECT * FROM [{0}coupontypes] WHERE [coupontypeid]=@coupontypeid AND [state]=1",
                                                RDBSHelper.RDBSTablePre);
            return RDBSHelper.ExecuteReader(CommandType.Text, commandText, parms);
        }
        ...
        public IDataReader AdminGetCouponTypeById(int couponTypeId)
        {
            ...
            string commandText = string.Format("SELECT * FROM [{1}coupontypes] WHERE [coupontypeid]=@coupontypeid",
        ...
        public void CreateCoupon(CouponInfo couponInfo)
        {
            DbParameter[] parms = {
                                    ...
                                    GenerateInParam("@validtime", SqlDbType.DateTime,8,couponInfo.ValidTime),
                                    
                                    GenerateInParam("@channelid", SqlDbType.VarChar,500,couponInfo.ChannelId)
                                    };
