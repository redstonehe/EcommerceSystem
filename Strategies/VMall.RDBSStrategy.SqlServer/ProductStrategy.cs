...
namespace VMall.RDBSStrategy.SqlServer
{
    ...
    public partial class RDBSStrategy : IRDBSStrategy
    {
        ...
        public DataTable AdminGetBrandList(int pageSize, int pageNumber, string condition, string sort)
        {
            ...
            string commandText = @"SELECT * FROM (	SELECT ROW_NUMBER() OVER (ORDER BY  " + sort + ") AS [rowid],a.* FROM [hlh_brands] a  WHERE " + condition + ") AS [temp] WHERE [rowid] BETWEEN " +( pageSize * (pageNumber - 1) + 1 )+ " AND "+pageSize * pageNumber ;
            //if (pageNumber == 1)
            //{
            //    if (noCondition)
            //        commandText = string.Format("SELECT TOP {0} {2} FROM [{1}brands] a ORDER BY {3}",
            //                                    pageSize,
            //                                    RDBSHelper.RDBSTablePre,
            //                                    RDBSFields.BRANDS,
            //                                    sort);
            ...
            //    else
            //        commandText = string.Format("SELECT TOP {0} {3} FROM [{1}brands] a WHERE {2} ORDER BY {4}",
            //                                    pageSize,
            //                                    RDBSHelper.RDBSTablePre,
            //                                    condition,
            //                                    RDBSFields.BRANDS,
            ...
            //{
            //    if (noCondition)
            //        commandText = string.Format("SELECT TOP {0} {3} FROM [{1}brands] a WHERE [brandid] NOT IN (SELECT TOP {2} [brandid] FROM [{1}brands] ORDER BY {4}) ORDER BY {4}",
            //                                    pageSize,
            //                                    RDBSHelper.RDBSTablePre,
            ...
            //                                    sort);
            //    else
            //        commandText = string.Format("SELECT TOP {0} {4} FROM [{1}brands] a WHERE [brandid] NOT IN (SELECT TOP {2} [brandid] FROM [{1}brands] WHERE {3} ORDER BY {5}) AND {3} ORDER BY {5}",
            //                                    pageSize,
            //                                    RDBSHelper.RDBSTablePre,
            //                                    (pageNumber - 1) * pageSize,
            //                                    condition,
        ...
        {
            bool noCondition = string.IsNullOrWhiteSpace(condition);
            string commandText = @"SELECT * FROM (	SELECT ROW_NUMBER() OVER (ORDER BY [brandid] DESC ) AS [rowid],a.* FROM [hlh_brands] a  WHERE " + condition + ") AS [temp] WHERE [rowid] BETWEEN " + (pageSize * (pageNumber - 1) + 1) + " AND " + pageSize * pageNumber;
            //if (pageNumber == 1)
            //{
            //    if (noCondition)
            //        commandText = string.Format("SELECT TOP {0} [brandid],[name] FROM [{1}brands] ORDER BY [brandid] DESC",
            //                                    pageSize,
            //                                    RDBSHelper.RDBSTablePre);

            //    else
            //        commandText = string.Format("SELECT TOP {0} [brandid],[name] FROM [{1}brands] WHERE {2} ORDER BY [brandid] DESC",
            //                                    pageSize,
            //                                    RDBSHelper.RDBSTablePre,
            //                                    condition);
            //}
            ...
            //{
            //    if (noCondition)
            //        commandText = string.Format("SELECT TOP {0} [brandid],[name] FROM [{1}brands] WHERE [brandid] NOT IN (SELECT TOP {2} [brandid] FROM [{1}brands] ORDER BY [brandid] DESC) ORDER BY [brandid] DESC",
            //                                    pageSize,
            //                                    RDBSHelper.RDBSTablePre,
            //                                    (pageNumber - 1) * pageSize);
            //    else
            //        commandText = string.Format("SELECT TOP {0} [brandid],[name] FROM [{1}brands] WHERE [brandid] NOT IN (SELECT TOP {2} [brandid] FROM [{1}brands] WHERE {3} ORDER BY [brandid] DESC) AND {3} ORDER BY [brandid] DESC",
            //                                    pageSize,
            //                                    RDBSHelper.RDBSTablePre,
            //                                    (pageNumber - 1) * pageSize,
            //                                    condition);
        ...
        public int AdminGetBrandCount(string condition)
        {
            ...
            if (string.IsNullOrWhiteSpace(condition))
                commandText = string.Format("SELECT COUNT(brandid) FROM [{0}brands] a", RDBSHelper.RDBSTablePre);
            else
                commandText = string.Format("SELECT COUNT(brandid) FROM [{0}brands] a WHERE {1}", RDBSHelper.RDBSTablePre, condition);
        ...
        {
            string commandText = string.Format("SELECT * FROM [{0}categories] ORDER BY [displayorder] DESC",
                                                RDBSHelper.RDBSTablePre,
        ...
        public int CreateCategory(CategoryInfo categoryInfo)
        {
            DbParameter[] parms = {
                                        ...
                                        GenerateInParam("@channelid", SqlDbType.VarChar,100, categoryInfo.ChannelId)
                                    };
            string commandText = string.Format("INSERT INTO [{0}categories]([displayorder],[name],[pricerange],[parentid],[layer],[haschild],[path],[channelid]) VALUES(@displayorder,@name,@pricerange,@parentid,@layer,@haschild,@path,@channelid);SELECT SCOPE_IDENTITY()",
                                                RDBSHelper.RDBSTablePre);
        ...
        public void UpdateCategory(CategoryInfo categoryInfo)
        {
            ...
            string commandText = string.Format("UPDATE [{0}categories] SET [displayorder]=@displayorder,[name]=@name,[pricerange]=@pricerange,[parentId]=@parentId,[layer]=@layer,[haschild]=@haschild,[path]=@path,[channelid]=@channelid WHERE [cateid]=@cateid",
                                                RDBSHelper.RDBSTablePre);
            RDBSHelper.ExecuteNonQuery(CommandType.Text, commandText, parms);
        }
        ...
        /// <returns></returns>
        public string AdminGetProductListCondition(int storeId, int storeCid, string productName, int cateId, int brandId, int state,int pid=0)
        {
        ...
        public DataTable AdminGetProductList(int pageSize, int pageNumber, string condition, string sort)
        {
            ...
            {
                if (noCondition)
                    commandText = string.Format("SELECT [temp1].[pid],[temp1].[storeid],[temp1].[name] AS [pname],[temp1].[shopprice],[temp1].[marketprice],[temp1].[state],[temp1].[isbest],[temp1].[ishot],[temp1].[isnew],[temp1].[displayorder],[temp1].[showimg],[temp1].[reviewcount],[temp1].[pv],[temp1].[haimi],[temp1].[addtime],[temp2].[number],[temp3].[name] AS [sname],[temp3].[mallsource] FROM (SELECT TOP {0} [pid],[storeid],[name],[shopprice],[marketprice],[state],[isbest],[ishot],[isnew],[displayorder],[showimg],[reviewcount],[pv],[haimi],[addtime] FROM [{1}products] ORDER BY {2}) AS [temp1] LEFT JOIN [{1}productstocks] AS [temp2] ON [temp1].[pid]=[temp2].[pid] LEFT JOIN [{1}stores] AS [temp3] ON [temp1].[storeid]=[temp3].[storeid] where [temp3].[mallsource] in (0,2,3)",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre,
                                                sort);

                else
                    commandText = string.Format("SELECT [temp1].[pid],[temp1].[storeid],[temp1].[name] AS [pname],[temp1].[shopprice],[temp1].[marketprice],[temp1].[state],[temp1].[isbest],[temp1].[ishot],[temp1].[isnew],[temp1].[displayorder],[temp1].[showimg],[temp1].[reviewcount],[temp1].[pv],[temp1].[haimi],[temp1].[addtime],[temp2].[number],[temp3].[name] AS [sname],[temp3].[mallsource] FROM (SELECT TOP {0} [pid],[storeid],[name],[shopprice],[marketprice],[state],[isbest],[ishot],[isnew],[displayorder],[showimg],[reviewcount],[pv],[haimi],[addtime] FROM [{1}products] WHERE {3} ORDER BY {2}) AS [temp1] LEFT JOIN [{1}productstocks] AS [temp2] ON [temp1].[pid]=[temp2].[pid] LEFT JOIN [{1}stores] AS [temp3] ON [temp1].[storeid]=[temp3].[storeid] where [temp3].[mallsource] in (0,2,3)",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre,
                                                sort,
                                                condition);
            ...
            {
                if (noCondition)
                    commandText = string.Format("SELECT [temp1].[pid],[temp1].[storeid],[temp1].[name] AS [pname],[temp1].[shopprice],[temp1].[marketprice],[temp1].[state],[temp1].[isbest],[temp1].[ishot],[temp1].[isnew],[temp1].[displayorder],[temp1].[showimg],[temp1].[reviewcount],[temp1].[pv],[temp1].[haimi],[temp1].[addtime],[temp2].[number],[temp3].[name] AS [sname],[temp3].[mallsource] FROM (SELECT TOP {0} [pid],[storeid],[name],[shopprice],[marketprice],[state],[isbest],[ishot],[isnew],[displayorder],[showimg],[reviewcount],[pv],[haimi],[addtime] FROM [{1}products] WHERE [pid] NOT IN (SELECT TOP {3} [pid] FROM [{1}products] ORDER BY {2}) ORDER BY {2}) AS [temp1] LEFT JOIN [{1}productstocks] AS [temp2] ON [temp1].[pid]=[temp2].[pid] LEFT JOIN [{1}stores] AS [temp3] ON [temp1].[storeid]=[temp3].[storeid] where [temp3].[mallsource] in (0,2,3)",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre,
                                                sort,
                                                (pageNumber - 1) * pageSize);
                else
                    commandText = string.Format("SELECT [temp1].[pid],[temp1].[storeid],[temp1].[name] AS [pname],[temp1].[shopprice],[temp1].[marketprice],[temp1].[state],[temp1].[isbest],[temp1].[ishot],[temp1].[isnew],[temp1].[displayorder],[temp1].[showimg],[temp1].[reviewcount],[temp1].[pv],[temp1].[haimi],[temp1].[addtime],[temp2].[number],[temp3].[name] AS [sname],[temp3].[mallsource] FROM (SELECT TOP {0} [pid],[storeid],[name],[shopprice],[marketprice],[state],[isbest],[ishot],[isnew],[displayorder],[showimg],[reviewcount],[pv],[haimi],[addtime] FROM [{1}products] WHERE [pid] NOT IN (SELECT TOP {3} [pid] FROM [{1}products] WHERE {4} ORDER BY {2}) AND {4} ORDER BY {2}) AS [temp1] LEFT JOIN [{1}productstocks] AS [temp2] ON [temp1].[pid]=[temp2].[pid] LEFT JOIN [{1}stores] AS [temp3] ON [temp1].[storeid]=[temp3].[storeid] where [temp3].[mallsource] in (0,2,3)",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre,
                                                sort,
                                                (pageNumber - 1) * pageSize,
        ...
        public int CreateProduct(ProductInfo productInfo)
        {
            DbParameter[] parms = {
                                     ...
                                     GenerateInParam("@pv",SqlDbType.Money,0,productInfo.PV),
                                     GenerateInParam("@haimi",SqlDbType.Money,0,productInfo.HaiMi),
                                     GenerateInParam("@saletype",SqlDbType.Int,1,productInfo.SaleType),
                                     GenerateInParam("@taxrate",SqlDbType.Float,4,productInfo.TaxRate),
                                     GenerateInParam("@hongbaocut",SqlDbType.Decimal,4,productInfo.HongBaoCut),
                                     GenerateInParam("@productparam",SqlDbType.NText,0,productInfo.ProductParam),
                                     GenerateInParam("@mobiledescription",SqlDbType.NText,0,productInfo.MobileDescription),
                                     GenerateInParam("@minbuycount",SqlDbType.Int,4,productInfo.MinBuyCount),
                                     GenerateInParam("@unit",SqlDbType.NVarChar,50,productInfo.Unit),
                                     GenerateInParam("@settlepercent",SqlDbType.TinyInt,1,productInfo.SettlePercent),
                                     GenerateInParam("@rebuycycle",SqlDbType.SmallInt,2,productInfo.ReBuyCycle),
                                     GenerateInParam("@relaterebuypid",SqlDbType.Int,4,productInfo.RelateReBuyPid),
                                     GenerateInParam("@videourl",SqlDbType.NChar,500,productInfo.VideoUrl)
                                   };

            string commandText = string.Format(@"INSERT INTO [{0}products]([psn],[cateid],[brandid],[storeid],[storecid],[storestid],[skugid],[name],[shopprice],[marketprice],[costprice],[state],[isbest],[ishot],[isnew],[weight],[displayorder],[showimg],[salecount],[visitcount],[reviewcount],[star1],[star2],[star3],[star4],[star5],[addtime],[description],[pv],[haimi],[saletype],[taxrate],[hongbaocut],[productparam],[mobiledescription],[minbuycount],[unit],[settlepercent],[rebuycycle],[relaterebuypid],[videourl])  
                                                ...
                                                @weight,@displayorder,@showimg,@salecount,@visitcount,@reviewcount,@star1,@star2,@star3,@star4,@star5,@addtime,@description,@pv,@haimi,@saletype,@taxrate,@hongbaocut,@productparam,@mobiledescription,@minbuycount,@unit,@settlepercent,@rebuycycle,@relaterebuypid,@videourl);
        ...
        public void UpdateProduct(ProductInfo productInfo)
        {
            DbParameter[] parms = {
                                    ...
                                    GenerateInParam("@pv",SqlDbType.Money,4,productInfo.PV),
                                    GenerateInParam("@haimi",SqlDbType.Money,4,productInfo.HaiMi),
                                    GenerateInParam("@saletype",SqlDbType.Int,1,productInfo.SaleType),
                                    GenerateInParam("@taxrate",SqlDbType.Decimal,4,productInfo.TaxRate),
                                    GenerateInParam("@hongbaocut",SqlDbType.Decimal,4,productInfo.HongBaoCut),
                                    GenerateInParam("@productparam",SqlDbType.NText,0,productInfo.ProductParam),
                                    GenerateInParam("@mobiledescription",SqlDbType.NText,0,productInfo.MobileDescription),
                                    GenerateInParam("@minbuycount",SqlDbType.Int,4,productInfo.MinBuyCount),
                                    GenerateInParam("@subtitle",SqlDbType.NVarChar,200,productInfo.SubTitle),
                                    GenerateInParam("@unit",SqlDbType.NVarChar,50,productInfo.Unit),
                                    GenerateInParam("@settlepercent",SqlDbType.TinyInt,1,productInfo.SettlePercent),
                                    GenerateInParam("@rebuycycle",SqlDbType.SmallInt,2,productInfo.ReBuyCycle),
                                    GenerateInParam("@relaterebuypid",SqlDbType.Int,4,productInfo.RelateReBuyPid),
            ...
            string commandText = string.Format(@"UPDATE [{0}products] SET [psn]=@psn,[cateid]=@cateid,[brandid]=@brandid,[storeid]=@storeid,[storecid]=@storecid,[storestid]=@storestid,[skugid]=@skugid,[name]=@name,[shopprice]=@shopprice,[marketprice]=@marketprice,[costprice]=@costprice,
                                                 ...
                                                 [star1]=@star1,[star2]=@star2,[star3]=@star3,[star4]=@star4,[star5]=@star5,[addtime]=@addtime,[description]=@description,[pv]=@pv,[haimi]=@haimi,[saletype]=@saletype,[taxrate]=@taxrate,[hongbaocut]=@hongbaocut,[productparam]=@productparam,[mobiledescription]=@mobiledescription,[minbuycount]=@minbuycount,[subtitle]=@subtitle,[unit]=@unit,[settlepercent]=@settlepercent,[rebuycycle]=@rebuycycle,[relaterebuypid]=@relaterebuypid,[videourl]=@videourl 
                                                 WHERE [pid]=@pid",
                                                 RDBSHelper.RDBSTablePre);
        ...
        public IDataReader AdminGetProductById(int pid)
        {
            ...
            string commandText = string.Format("SELECT * FROM [{1}products] WHERE [pid]=@pid",
                                                RDBSFields.PRODUCTS,
                                                RDBSHelper.RDBSTablePre);
            return RDBSHelper.ExecuteReader(CommandType.Text, commandText, parms);
        }
        ...
        public IDataReader AdminGetPartProductById(int pid)
        {
            ...
            string commandText = string.Format("SELECT * FROM [{0}products] WHERE [pid]=@pid",
                                                RDBSHelper.RDBSTablePre);
            return RDBSHelper.ExecuteReader(CommandType.Text, commandText, parms);
        }
        ...
        public IDataReader GetStoreClassProductList(int pageSize, int pageNumber, int storeCid, int startPrice, int endPrice, int sortColumn, int sortDirection)
        {
            ...
            if (pageNumber == 1)
            {
                commandText.AppendFormat("SELECT TOP {1} * FROM [{0}products]", RDBSHelper.RDBSTablePre, pageSize);
                ...
                commandText.Append(" AND [displayorder]=1");
                commandText.Append(" ORDER BY ");
                switch (sortColumn)
                {
                    case 0:
            ...
            else
            {
                ...
                commandText.AppendFormat(") AS [rowid],* FROM [{0}products]", RDBSHelper.RDBSTablePre);
        ...
        {
            string commandText = string.Format("SELECT * FROM [{0}products] WHERE [pid] IN ({1}) ORDER BY CHARINDEX(','+CONVERT(varchar(10),[pid])+',',','+'{1}'+',')",
                                                RDBSHelper.RDBSTablePre,
                                                pidList);
            return RDBSHelper.ExecuteDataset(CommandType.Text, commandText).Tables[0];
        }
        ...
        /// </summary>
        /// <returns></returns>
        public IDataReader GetHomeProducts()
        {
            StringBuilder commandText = new StringBuilder();

            commandText.AppendFormat("SELECT TOP {1} [p].*,[s].[name] AS [storename] FROM [{0}products] AS [p]", RDBSHelper.RDBSTablePre, 20);

            commandText.AppendFormat(" LEFT JOIN [{0}stores] AS [s] ON [p].[storeid]=[s].[storeid]", RDBSHelper.RDBSTablePre);

            commandText.AppendFormat(" WHERE [p].[ishot]=1  AND [p].[state]=0  AND [s].[state]=0  ORDER BY [p].[salecount]  DESC");
        ...
        public void ReviewProduct(ProductReviewInfo productReviewInfo)
        {
            DbParameter[] parms = {
                                    ...
                                    GenerateInParam("@shownickname", SqlDbType.NVarChar, 20, productReviewInfo.ShowNickName)
                                    };
  ...
  SELECT ROW_NUMBER() OVER (ORDER BY {0}  ) AS [rowid],a.*,b.name as [sname]  FROM [hlh_productreviews] a LEFT JOIN dbo.hlh_stores b ON a.storeid = b.storeid   WHERE {1} and a.[parentid] = 0 
        ...
        /// <returns></returns>
        public string AdminGetProductReviewListCondition(int storeId, int pid, string pname, int type, string message, string startTime, string endTime)
        {
            StringBuilder condition = new StringBuilder(" a.[parentid] = 0 ");
            ...
            if (!string.IsNullOrEmpty(pname))
                condition.AppendFormat(" AND a.[pname] like '%{0}%' ", pname);
            if (type == 1)//好评
                condition.Append(" AND (a.[star]=5 OR a.[star]=4) ");
            else if (type == 2)//中评
                condition.Append(" AND (a.[star]=3 OR a.[star]=2) ");
            else if (type == 3)//差评
                condition.Append(" AND (a.[star]=1 OR a.[star]=0) ");

            if (!string.IsNullOrWhiteSpace(message))
                condition.AppendFormat(" AND a.[message] like '%{0}%' ", message);

            if (!string.IsNullOrEmpty(startTime))
                condition.AppendFormat(" AND a.[reviewtime] >= '{0}' ", TypeHelper.StringToDateTime(startTime).ToString("yyyy-MM-dd HH:mm:ss"));

            if (!string.IsNullOrEmpty(endTime))
                condition.AppendFormat(" AND a.[reviewtime] <= '{0}' ", TypeHelper.StringToDateTime(endTime).ToString("yyyy-MM-dd HH:mm:ss"));
        ...
        public string AdminGetProductReviewListSort(string sortColumn, string sortDirection)
        {
            ...
            return string.Format("a.{0} {1} ", sortColumn, sortDirection);
        }
        ...
        public int AdminGetProductReviewCount(string condition)
        {
            string commandText;
            if (string.IsNullOrWhiteSpace(condition))
                commandText = string.Format("SELECT COUNT(reviewid) FROM [{0}productreviews] a", RDBSHelper.RDBSTablePre);
            else
                commandText = string.Format("SELECT COUNT(reviewid) FROM [{0}productreviews] a WHERE {1}", RDBSHelper.RDBSTablePre, condition);
        ...
        public DataTable GetSearchWordStatList(int pageSize, int pageNumber, string word, string sort)
        {
            ...
            {
                if (noCondition)
                    commandText = string.Format("SELECT [word],[temp].[totaltimes],[lastsearchtime] FROM ( SELECT t.*,ROW_NUMBER() OVER (ORDER BY [totaltimes] DESC ) AS [rowid] FROM (SELECT TOP {2} [word],SUM([times]) AS [totaltimes],MAX([updatetime]) AS [lastsearchtime] FROM [{0}searchhistories] GROUP BY [word] ORDER BY {1})  AS t) AS [temp] WHERE [temp].[rowid] BETWEEN {3} AND {2}",
                                                RDBSHelper.RDBSTablePre,
                                                sort,
                                                pageNumber * pageSize,
                                                (pageNumber - 1) * pageSize + 1);
                else
                    commandText = string.Format("SELECT [word],[temp].[totaltimes],[lastsearchtime] FROM (SELECT t.*,ROW_NUMBER() OVER (ORDER BY [totaltimes] DESC ) AS [rowid] FROM (SELECT TOP {2} ROW_NUMBER() OVER (ORDER BY {1}) AS [rowid],[word],SUM([times]) AS [totaltimes],MAX([updatetime]) AS [lastsearchtime] FROM [{0}searchhistories] WHERE [word] LIKE '{4}%' GROUP BY [word] ORDER BY {1} )  AS t) AS [temp] WHERE [temp].[rowid] BETWEEN {3} AND {2}",
                                                RDBSHelper.RDBSTablePre,
                                                sort,
                                                pageNumber * pageSize,
                                                (pageNumber - 1) * pageSize + 1,
        ...
        {
//            SELECT MAX(T.row) FROM(
//SELECT COUNT([word]) AS w ,ROW_NUMBER() OVER (ORDER BY  [word]  DESC)  AS row FROM [hlh_searchhistories] WHERE [word] LIKE '手%' GROUP BY [word]
//) AS T
            string commandText;
            if (string.IsNullOrWhiteSpace(word))
                commandText = string.Format("SELECT MAX(T.row) FROM(SELECT COUNT([word]) AS w ,ROW_NUMBER() OVER (ORDER BY  [word]  DESC)  AS row FROM [{0}searchhistories]  GROUP BY [word]) AS T", RDBSHelper.RDBSTablePre);
            else
                commandText = string.Format("SELECT MAX(T.row) FROM(SELECT COUNT([word]) AS w ,ROW_NUMBER() OVER (ORDER BY  [word]  DESC)  AS row FROM [{0}searchhistories] WHERE [word] LIKE '{1}%' GROUP BY [word]) AS T", RDBSHelper.RDBSTablePre, word);
            return TypeHelper.ObjectToInt(RDBSHelper.ExecuteScalar(CommandType.Text, commandText));
        }
