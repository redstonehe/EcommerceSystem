...
namespace VMall.CartStrategy.SqlServer
{
    /// <summary>
    /// 基于SqlServer的购物车策略
    /// </summary>
    public partial class CartStrategy : ICartStrategy
    {
        ...
        public int GetCartProductCount(int uid)
        {
            ...
            string commandText = string.Format("SELECT SUM([realcount]) FROM [{0}orderproducts] WHERE [uid]=@uid AND [oid]=0", RDBSHelper.RDBSTablePre);
            return TypeHelper.ObjectToInt(RDBSHelper.ExecuteScalar(CommandType.Text,
                                                                  commandText,
                                                                   parms), 0);
        ...
        public int GetCartProductCount(string sid)
        {
            ...
            string commandText = string.Format("SELECT SUM([realcount]) FROM [{0}orderproducts] WHERE [sid]=@sid AND [oid]=0", RDBSHelper.RDBSTablePre);
            return TypeHelper.ObjectToInt(RDBSHelper.ExecuteScalar(CommandType.Text,
                                                                  commandText,
                                                                   parms), 0);
            //return TypeHelper.ObjectToInt(RDBSHelper.ExecuteScalar(CommandType.StoredProcedure,string.Format("{0}getcartproductcountbysid", RDBSHelper.RDBSTablePre),parms), -2);
        }
        ...
        public void AddOrderProductList(List<OrderProductInfo> orderProductList)
        {
            foreach (OrderProductInfo orderProductInfo in orderProductList)
            {
                DbParameter[] parms = {
                                        ...
                                        GenerateInParam("@mallsource", SqlDbType.Int, 4, orderProductInfo.MallSource)
                                    };
        ...
        /// <param name="uid">用户id</param>
        /// <returns></returns>
        public List<OrderProductInfo> GetCartProductList(int uid, int mallsource = 0)
        {
            ...
            string commandText = string.Format("SELECT * FROM [{0}orderproducts] WHERE [uid]=@uid AND [oid]=0 AND [mallsource]={1} ", RDBSHelper.RDBSTablePre, mallsource);
            IDataReader reader = RDBSHelper.ExecuteReader(CommandType.Text, commandText, parms);
            //RDBSHelper.ExecuteReader(CommandType.StoredProcedure,string.Format("{0}getcartproductlistbyuid", RDBSHelper.RDBSTablePre),parms);
            while (reader.Read())
            {
        ...
        /// <param name="sid">用户sid</param>
        /// <returns></returns>
        public List<OrderProductInfo> GetCartProductList(string sid, int mallsource = 0)
        {
            List<OrderProductInfo> orderProductList = new List<OrderProductInfo>();
            ...
            string commandText = string.Format("SELECT * FROM [{0}orderproducts] WHERE [sid]=@sid AND [oid]=0 AND [mallsource]={1} ", RDBSHelper.RDBSTablePre, mallsource);
            IDataReader reader = RDBSHelper.ExecuteReader(CommandType.Text, commandText, parms);
            // RDBSHelper.ExecuteReader(CommandType.StoredProcedure, string.Format("{0}getcartproductlistbysid", RDBSHelper.RDBSTablePre), parms);
            while (reader.Read())
            {
                OrderProductInfo orderProductInfo = BuildOrderProductFromReader(reader);
                orderProductList.Add(orderProductInfo);
